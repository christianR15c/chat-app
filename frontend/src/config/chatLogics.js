...
export const isSameSenderMargin = (messages, message, index, userId) => {
    if (
        ...
        index < messages.length - 1 && messages[index + 1].sender?._id !== message.sender?._id && messages[index].sender?._id !== userId || (index === messages.length - 1 && messages[index].sender?._id !== userId)
    ) return 0
    else return 'auto'
}
